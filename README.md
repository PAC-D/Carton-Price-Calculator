...
*   **Pricing Data Check** (`primark-pricing-data-check/`): Standalone look-up of packaging rates (US $ per SQM) by factory and supplier, with alphabetical sorting and a **Primark SQM column derived via the P/E model** — `Primark $/sqm = Supplier $/sqm × (Primark area ÷ Supplier area)` — using the real paper-consumption SQM formulas (M&U roll 2300, Uniglory roll 1600, Primark, Epyllion) for a fixed reference carton of 500 × 300 × 300. Primark is consistently cheaper since its SQM measures a smaller area. The table and branded PDF (both PACD + Primark logos) show 6 columns: SL, Packaging Supplier, Factory, Factory SQM Price, Supplier SQM (area), and Primark SQM Price. Pre-computed reports: `primark-sqm-500x300x300.csv` and `carton-prices-500x300x300.csv`. Rate data is maintained separately in `primark-pricing-data-check/data.csv`.
